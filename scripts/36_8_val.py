...
PAIRED_CSV = CASE_DIR / f"{HAR_ID}__paired_windows.csv"
...
OUT_DIR = CASE_DIR / "peak_model_validation"
...
def fetch_2114_sequence_from_window_row(
    species_label: str,
    chrom: str,
    genomic_window_start: int,
    genomic_window_end: int,
    start_in_500: int,
    end_in_500: int,
) -> tuple[str, dict]:
    """
    从 30bp window 的基因组坐标 + 它在500bp里的相对位置，
    反推出原始500bp region，再构造2114bp输入序列。
    """
    window_len = int(genomic_window_end) - int(genomic_window_start)
    expected_window_len = int(end_in_500) - int(start_in_500)

    if window_len != expected_window_len:
        warnings.warn(
            f"Window genomic length {window_len} != expected from in-seq coords {expected_window_len}"
        )

    region500_start = int(genomic_window_start) - int(start_in_500)
    region500_end = region500_start + CENTER_REGION_LEN

    start2114 = region500_start - FLANK_LEN
    end2114 = region500_end + FLANK_LEN

    if start2114 < 0:
        raise ValueError(
            f"2114 window start < 0 for {species_label} "
            f"{chrom}:{genomic_window_start}-{genomic_window_end}"
        )
    ...
    meta = {
        "chrom": chrom,
        "window_start": int(genomic_window_start),
        "window_end": int(genomic_window_end),
        "region500_start": region500_start,
        "region500_end": region500_end,
...
def summarize_prediction_vector(pred: np.ndarray) -> dict:
    ...
    argmax_ref = int(np.argmax(pred))
    return {
        "pred_mean": float(np.mean(pred)),
        "pred_max": float(np.max(pred)),
        "pred_sum": float(np.sum(pred)),
        "pred_argmax": argmax_ref,
...
def run_single_case(row: pd.Series, human_peak_model, macaque_peak_model):
    ...
    chrom_h = str(row["chrom__human"])
    chrom_m = str(row["chrom__macaque"])

    genomic_h_start = int(row["genomic_start_0based__human"])
    genomic_h_end = int(row["genomic_end_0based_exclusive__human"])
    genomic_m_start = int(row["genomic_start_0based__macaque"])
    genomic_m_end = int(row["genomic_end_0based_exclusive__macaque"])
    ...
    full_h_2114, _ = fetch_2114_sequence_from_window_row(
        species_label="human",
        chrom=chrom_h,
        genomic_window_start=genomic_h_start,
        genomic_window_end=genomic_h_end,
        start_in_500=h_s_500,
        end_in_500=h_e_500,
    )

    full_m_2114, _ = fetch_2114_sequence_from_window_row(
        species_label="macaque",
        chrom=chrom_m,
        genomic_window_start=genomic_m_start,
        genomic_window_end=genomic_m_end,
        start_in_500=m_s_500,
        end_in_500=m_e_500,
    )
    ...
    def add_row(model_name, seq_name, pred_ref, pred_mut):
        ...
        argmax_ref = s_ref["pred_argmax"]
        delta_at_argmax_ref = float(pred_mut[argmax_ref] - pred_ref[argmax_ref])
        max_abs_change = float(np.max(np.abs(pred_mut - pred_ref)))
        ...
        rows.append({
            ...
            "delta_at_argmax_ref": delta_at_argmax_ref,
...
def main():
    ...
    if pair_df.empty:
        raise ValueError("Paired CSV is empty")
    ...
    if len(all_summary) > 0:
        ...
        out_summary = OUT_DIR / f"{HAR_ID}__peak_model_validation_summary.csv"
