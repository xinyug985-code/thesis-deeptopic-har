...
SENSITIVE_CSV = ISM_DIR / "34_1_top_sensitive_sites_high_only.csv"

OUT_POS_CSV = ISM_DIR / "34_2_sensitive_vs_mutation_positions.csv"
OUT_SUMMARY_CSV = ISM_DIR / "34_2_sensitive_vs_mutation_summary.csv"
...
def main():
    ...
    for _, row in align_df.iterrows():
        ...
        # human 
        human_len = len(seq_h)
        macaque_len = len(seq_m)

        # union 
        all_aln_positions = sorted(set(maps["aln_to_human_raw"].keys()) | set(maps["aln_to_macaque_raw"].keys()))

        pair_overlap_count = 0
        pair_sensitive_h_count = 0
        pair_sensitive_m_count = 0
        pair_mismatch_count = len(mismatch_aln_positions)
